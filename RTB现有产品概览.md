...
> **运行时补全**：§2.11 支出管理另据 MuMu（`emulator-5554` · 包 `com.jiyong.rta.next.debug`）`uiautomator` / 截图实测，标注「MuMu 实测」。  
...
- **覆盖（对照重构席位）**：客户 · 预约 · 价目（项目与产品） · 会员卡 · 优惠券 · 开单 · 流水 · 经营分析 · 员工管理（含排班能力若有则并入口径） · 支出管理 · 库存（并入项目与产品）
...
| 支出管理 | ≥3（实测） | — | 门店支出记账、分类管理、月汇总；统计为 H5 | 工作台「支出管理」→ `PayHomeActivity` |
...
- 前线：首页「门店经营/业绩构成/项目排名」跳转期望跟首页改版（暂不修复）。
...
- 前线：自定义结算周期进行中；提成明细可读性 / 老板端下钻待开始（第四章）。
- 现网推断。
...
> **MuMu 实测**（2026-08-11）：包 `com.jiyong.rta.next.debug`；入口已在工作台打开「支出管理」。APK 分析模块地图仍无独立「支出」归类，包名实测为 `com.rts.rta.paymanage`。本次 **未保存/未删除** 任何支出或分类。
...
%%{init: {'flowchart': {'nodeSpacing': 24, 'rankSpacing': 32, 'padding': 10, 'htmlLabels': true}, 'themeVariables': {'fontSize': '18px', 'fontFamily': 'PingFang SC, Microsoft YaHei, sans-serif'}}}%%
flowchart TB
  subgraph laneHome [① 首页汇总]
    direction LR
    home[支出管理] --> month[月份筛选]
    home --> tip[现金类总收款说明]
    home --> report[门店经营报表]
    home --> list[支出列表或空态]
  end

  subgraph laneBook [② 记一笔]
    direction LR
    home2[支出管理] --> book[记一笔]
    book --> cats[分类管理]
    cats --> addCat[添加分类弹层]
  ...
  subgraph laneStat [③ 看统计]
    direction LR
    home3[支出管理] --> stats[支出统计页]
    stats --> trend[支出趋势]
    stats --> compose[支出构成]
  end

  laneHome ~~~ laneBook ~~~ laneStat
```

#### 关键支线

- **首页**：月维度切换（前/后一月）；汇总卡「支出」+「现金类总收款」；「?」提示文案为「门店经营 项目、开续卡获得的现金流收入」；列表无数据时空态「暂无数据」。
- **记一笔**（`PayBookkeepingActivity`）：顶栏日期；金额数字键盘 +「确定」；备注「请输入备注」；类型宫格默认含：进货成本、房租、水电、店内支出、奖金、工资发放、餐饮、交通、团建、其它，另有「分类管理」。
- **分类管理**（`SortManageActivity`）：提示「长按拖动可调整位置 点击自定义的分类可删除或编辑」；「添加分类」底栏弹层（选图标 + 名称「不能与已有分类重名，限1~4字」+「确定」/关闭）。
- **看统计**：进入 `StoreDataStatisticsActivity`（H5「web-剑琅管家」），仍标题「支出管理」；含前/后一月、支出 / 现金类总收款 / 结余、支出趋势（年）、支出构成。
- **查看门店经营报表**：同 Activity 族 H5，打开「门店经营报表」（数据总览 / 实收分析 / 员工业绩分析 / 卡相关 / 客流分析等），属经营分析交叉入口。
- **未进入**：本店当月无支出记录，**列表行详情 / 改删** 未实测；有数据时列表应可点入（推断）。
- 对照重构：重构原型另有独立「支出流水」页；现网首页即列表 +「看统计」H5，信息架构不完全同构。

#### 页面清单速查

| 代表页面 | 说明 |
| --- | --- |
| PayHomeActivity | 支出管理首页（月汇总 + 列表 + 记一笔/看统计） |
| PayBookkeepingActivity | 记一笔（金额 / 类型 / 日期 / 备注） |
| SortManageActivity | 分类管理（排序 / 添加分类弹层） |
| StoreDataStatisticsActivity | 「看统计」与「门店经营报表」H5 容器（剑琅管家） |
...
- **员工提成**：自定义结算周期 → 进行中；明细可读性、老板端下钻 → 待开始
- **预约通知**：成功后店铺消息 → 待开始
- **已关闭/暂不修**：美发师无券设置（设计如此）；首页报表跳转、散客性别默认、减少开单签名次数、积分兑换 → 暂不修复
...
**汇总**：进行中 4 · 待开始 3 · 已完成 3 · 已关闭 1 · 暂不修复 4；**待办 7**（进行中+待开始）；**无延期项**。  
快照非实时看板。
...
| 序号 | 问题描述 | 门店 | 类型 | 处理进度 | 关联模块 |
| ---: | --- | --- | --- | --- | --- |
| 5 | 员工业绩提成明细时间模块颜色需要加深 | 艾美造型（喜哥） | 优化 | （无） | 员工 / 流水 |
| 6 | 老板端门店业绩页应可直接点击员工查看业绩提成 | 艾美造型（喜哥） | 需求 | 如何更方便地看到（待确认方案） | 经营分析 / 员工 |
| 7 | 预约成功后店铺增加消息通知功能 | 艾美造型（喜哥） | 需求 | 推送（待开始） | 预约 / 消息 |

### 4.3 已完成（3）

| 序号 | 问题描述 | 门店 | 类型 | 处理进度 | 关联模块 |
| ---: | --- | --- | --- | --- | --- |
| 8 | 会员卡增加项目减金额的设置 | 高美丝造型 | 需求 | 已完成 | 会员卡 |
| 9 | 项目已添加为卡项后，修改项目名称无法修改 | 销售体验 | 优化 | 已完成 | 价目 / 会员卡 |
| 10 | 设置项目时选错所属项目大类无法更改 | 销售体验 | 优化 | 已完成 | 价目 |

### 4.4 已关闭（1）

| 序号 | 问题描述 | 门店 | 类型 | 处理进度 | 关联模块 |
| ---: | --- | --- | --- | --- | --- |
| 11 | 美发师没有优惠券设置功能 | 艾美造型（喜哥） | 需求 | 已关闭（产品设计如此） | 优惠券 |

### 4.5 暂不修复（4）

| 序号 | 问题描述 | 门店 | 类型 | 处理进度 | 关联模块 |
| ---: | --- | --- | --- | --- | --- |
| 12 | 首页「门店经营/业绩构成/项目排名」跳转业绩板块而非详情页 | 销售体验 | 需求 | leon：跟首页改版来做 | 工作台 / 经营分析 |
| 13 | 散客大部分为男性但开单默认被选为女性 | 艾美造型（喜哥） | 优化 | 系统默认女性可手动修改，沿用上次散客性别 | 开单 / 客户 |
...
| 报表统计 | 13 | 8 | 经营分析 |
| 员工管理 | 10 | 15 | 员工管理 |
| （APK 分析无独立归类 · `paymanage`） | ≥3（MuMu） | — | 支出管理（实测补全） |
