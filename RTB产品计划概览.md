...
- **端**：商户端 B 端（手机框原型）
- **覆盖**：客户 · 预约 · 价目 · 会员卡 · 优惠券 · 开单 · 流水 · 经营分析 · 员工管理 · 排班 · 支出管理 · 库存管理
...
| 5 | 员工管理 | 开单分账/业绩、权限；排班与薪资前置 |
| 6 | 订单流水 | 依赖开单落单；退改/尾款在真实单据上测 |
| 7 | 优惠券管理 | 发放→客户；核销→开单；标的可挂已有价目 |
| 8 | 员工排班 | 依赖员工档案；再支撑预约 |
| 9 | 预约管理 | 依赖客户 + 排班（+ 价目服务项） |
| 10 | 经营分析 | 依赖流水/开单数据沉淀 |
| 11 | 支出管理 | 弱依赖交易主链；经手人可挂员工 |
| 12 | 库存管理 | 付费升级；开通后关联价目「产品」，宜主交易稳定后 |

---

## 五、附录：FLOW 全量索引

已落地 **128** 个地图节点（含浮层 / Dialog 深链态）；客户 / 预约 / 优惠券 / 员工排班为占位分组。顺序：计划模块序（占位穿插）+ 其余与原型 FLOW 地图一致。
