...
2. 价目分 **项目** 与 **产品** 两个 Tab；均可 **分组筛选**、**多分组归属**、行内 **拖拽排序**（在约束内）、左滑/长按快捷操作。列表为 **在售区 →「已下架」分隔行 →「已隐藏」折叠**，**无状态列**、**无搜索**；新建入口为标题栏「+添加」与有数据时的悬浮球。  
...
- 价目表列表：项目 Tab / 产品 Tab、标题栏「+添加」、有数据时悬浮球「+」、列头排序、空态（**无搜索**）  
...
| 页面/层 | 导航标题（原型） | 说明 |
|---------|------------------|------|
| 列表 | 价目表 | Tab 项目/产品；标题栏「+添加」+ 悬浮球；项目列名称/时长/价格；产品列名称（规格）/价格 |
