...
2. 价目分 **项目** 与 **产品** 两个 Tab；均可 **分组筛选**、**多分组归属**、行内 **拖拽排序**（在约束内）、左滑/长按快捷操作。列表为 **在售区 →「已下架」分隔行 →「已隐藏」折叠**，**无状态列**、**无搜索**。新建：标题栏「+添加」与有数据时的悬浮球均 **跟当前 Tab 直接新建**（项目 Tab→新增项目，产品 Tab→新增产品），**不弹选择层**。  
...
- 价目表列表：项目 Tab / 产品 Tab、标题栏「+添加」（跟 Tab 直接新建、不弹窗）、有数据时悬浮球「+」、列头排序、空态（**无搜索**）  
...
| 页面/层 | 导航标题（原型） | 说明 |
|---------|------------------|------|
| 列表 | 价目表 | Tab 项目/产品；标题栏「+添加」跟 Tab 直接新建（不弹窗）+ 悬浮球同规则；项目列名称/时长/价格；产品列名称（规格）/价格 |
