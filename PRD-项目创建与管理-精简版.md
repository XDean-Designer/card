...
7. **可见性**：顾客端 / **开单选品** / **建卡权益选品** 均为 `onSale && !hidden`。开单中价目改价 → 支付 Dialog「支付金额已变化」（`bill-pay-price-changed`，见完整稿 §6.8）。  
...
| 新建产品 | 填名称/规格（选填）/价格后出现在产品列表 | 列表「名称（规格）」+ 价格；在售未隐藏时可开单/权益选用 |
...
| 开单选品 | 开单只选可售货 | 仅 `onSale && !hidden`；价目产品不限购 |
