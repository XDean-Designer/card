...
2. 价目分 **项目** 与 **产品** 两个 Tab；均可 **分组筛选**、**多分组归属**、行内 **拖拽排序**（在约束内）、左滑/长按快捷操作。列表为 **在售区 →「已下架」分隔行 →「已隐藏」折叠**，**无状态列**、**无搜索**。新建：标题栏品牌红「+添加」按钮 **跟当前 Tab 直接新建**（项目 Tab→新增项目，产品 Tab→新增产品），**不弹选择层**；**无右下悬浮球**。项目 **隐藏** 时自动关闭「可预约」；**取消隐藏后仍关**，须手动再开。  
3. 项目可被会员卡权益 **按名称绑定**（同桶 **名称唯一**）；绑卡后 **锁定名称、可预约、删除**；**在售/下架仍可改**（细则见完整稿）。`boundToCard` **自动派生刷新**。删除锁定以锁标呈现。  
4. B 端改价、上下架、时长、**隐藏**、封面等变更需 **同步客户端展示**（开单为 **实时目录/价格**；细节【待联调】）；顾客端具体 UI **不在本文展开**。价目产品本期不维护库存。封面正式 OSS **本期不做**。  
5. 会员卡建卡时「选项目 / 选产品 / 选折扣适用项目」及标题栏「管理价目表」依赖本价目数据；从权益页进价目再返回时 **刷新分组 Tab（重置为「全部」）**；会员卡三步流程见 `PRD-会员卡管理.md`，本文只写联结面。  
6. **产品库存**（以当前原型为准；完整稿 **8.1**）：价目产品 **无库存字段/列表列**；开单对价目产品 **不限购**；后续库存管理另模块。  
7. **可见性**：顾客端 / 建卡权益选品为 `onSale && !hidden`；**开单选品** 为 `onSale`（**含隐藏项**；已下架不可选）。开单中价目改价 → 支付 Dialog「支付金额已变化」（`bill-pay-price-changed`，见完整稿 §6.8）。  
8. 原型工具栏「无数据 / 有数据」造数 **仅演示**，不进正式产品。接口路径标 `【待联调确认】`，正式文档 **暂无交付时间**。  
9. **本期仅单店**；**店主可编辑价目，店员不可编辑**。  
10. **输入边界**：名称 ≤30；价格 0～999,999.99；开单无库存上限时单品数量 ≤999（完整稿 **5.1**）。
...
| 可见性清晰 | 顾客端 / 开单 / 建卡选品范围分明（**隐藏项开单可选**；权益仍须未隐藏） |
| 与卡对齐 | 绑卡锁定规则清晰；权益选品只取在售且未隐藏 |

## 1.3 本期做（In）

- 价目表列表：项目 Tab / 产品 Tab、标题栏品牌红「+添加」按钮（跟 Tab 直接新建、不弹窗；**无悬浮球**）、列表底部虚线添加行、列头排序、空态（**无搜索**）  
- 列表分区：**在售区** → **「已下架」分区** →（「全部」）**「已隐藏」折叠**（无数量）  
- 新增 / 编辑 / 删除项目与产品（表单：基本信息 / 售卖设置 / 高级设置；产品含 **规格选填**；封面图选填最多 1 张；**无库存字段**）  
- **产品库存**：价目侧本期不维护（先读第 6 条 / 完整稿 **8.1**）  
- 在售切换、**隐藏 / 取消隐藏**  
...
- 项目绑卡锁定（部分字段）及提示文案  
- 从会员卡权益页「管理价目表」进入并返回  
- **开单选品** 联结（在售即可，**含隐藏**；价目改价支付拦截见完整稿）  
- B→C 同步生效的产品语义说明（无 C 端页面规格）

## 1.4 本期不做（Out）

- 顾客端 / 小程序预约页、开单页的完整 UI 规格（仅约定消费价目结果）  
- **「库存管理」完整功能**（入库/出库/盘点等）：**不在本期价目表**；见完整稿 8.1  
...
| 新建产品 | 填名称/规格（选填）/价格后出现在产品列表 | 列表「名称（规格）」+ 价格；在售可开单（含隐藏）；权益须未隐藏 |
| 改价 | 绑卡或未绑卡均可改价（规则内） | toast 成功；开单中改价可触发「支付金额已变化」 |
| 下架 | 停售但仍可在价目表看到 | 落入「已下架」分区；顾客端 / 开单 / 权益均不可选 |
| 隐藏 | 对顾客端隐藏但 B 端保留 | 「全部」主列表不显示、底部「已隐藏」折叠可展开；「隐藏」Tab 可见；**开单可选**；权益选品不可选；**项目隐藏时关闭预约，取消隐藏后仍关，须手动再开** |
| 产品库存 | 价目侧不维护库存 | 无表单字段/列表列；开单对价目产品不限购；库存管理另模块（完整稿 8.1） |
| 分组运营 | 建组、把条目加入多组、按组筛选 | 筛选正确；删组不丢条目；系统「隐藏」组不可删/改名 |
| 拖拽排序 | 调整同售卖态桶内顺序 | 刷新后顺序保持；自定义分组内可拖；「隐藏」Tab / 「全部」已隐藏折叠 / 列排序激活时不可拖（**无搜索**） |
| 绑卡项目 | 被卡引用后不能乱改关键属性 | 锁定名称/预约/删除有 toast 与锁标；**可改在售/下架** |
| 开单选品 | 开单可选在售货（含对 C 端隐藏项） | **仅排除已下架**：`onSale`；隐藏项可选；价目产品不限购 |
