...
4. B 端改价、上下架、时长、隐藏、封面等变更需 **同步客户端展示**（开单侧为 **实时目录/价格**；推送细节【待联调确认】）；顾客端具体 UI **不在本文展开**。  
...
| 分区 | 字段 | 必填 | 默认（新增） | 说明 |
|------|------|:----:|--------------|------|
| 基本信息 | 名称 | ✓ | 空 | 表单标签文案为「项目名称」（与项目共用控件文案）；**同桶唯一**（trim 后）；校验空名时 toast「请填写产品名称」；重名 toast「名称已存在」 |
...
| **开单选品** | 通过 `CardCatalogBridge.getBillProjects` / `getBillProducts` 读取价目 **全量实时目录**（含已下架、已隐藏）；改价后开单侧可见最新价；缓存/推送实现【待联调确认】 |
...
4. **绑卡匹配**：以 **名称** 与模板权益引用对齐；**同桶名称唯一**；改名锁定避免孤儿引用；解除引用后 `boundToCard` **自动刷新**。  
...
10. **B→C / 开单同步**：价目变更后客户端与开单侧按 **实时目录/价格** 语义生效；推送与缓存细节【待联调确认】；顾客端页面结构不在范围。  
11. **列排序 vs 手动排序**：列排序为临时视图；手动拖拽前须取消列排序并回到「全部」分组。  
12. **产品无预约字段**；项目才有「手机端可预约」。  
13. **权益选品分组 Tab**：仅自定义组 +「全部」；**不展示** 系统「隐藏」Tab；从「管理价目表」返回后 **刷新分组 Tab（重置为「全部」）**。  
14. **封面图**：选填；演示最多 **1 张**；正式 OSS **本期不做**。  
15. **权限**：店主可编辑价目；店员不可编辑。  
16. **门店范围**：**本期仅单店**。
...
**Toast**：请输入分组名称、分组名称已存在、名称已存在、已重命名、已新建分组、已删除分组、已更新成员、已更新分组、已隐藏、已取消隐藏、已上架、已下架、保存成功、项目已创建、产品已创建、已删除、请填写项目名称、请填写产品名称、请填写有效价格、请填写项目时长、请清空搜索后再排序（分组筛选拦截拖拽时）、请先取消列排序后再手动排序、已添加演示图片、已移除图片、仅可上传 1 张封面；以及模块 5.5 绑卡/预约/删除拦截句。
...
15. **Given** 从添加项目权益进「管理价目表」并新增项目，**When** 返回，**Then** 回到权益页且可选到新在售未隐藏项目，**分组 Tab 刷新为「全部」**。  
...
19. **Given** 当前为项目 Tab 且列表有数据，**When** 点标题栏「+添加」，**Then** **不弹窗**，直接进入「新增项目」。  
20. **Given** 当前为产品 Tab 且列表有数据，**When** 点标题栏「+添加」，**Then** **不弹窗**，直接进入「新增产品」。  
21. **Given** 任意价目表列表态，**When** 查看界面，**Then** **无**右下悬浮球「+」；整表无数据时可用空态 CTA 添加，标题栏「+添加」始终可用。  
22. **Given** 同桶已存在名称「精致剪发」，**When** 再保存同名项目（trim 后相同），**Then** toast「名称已存在」且不写入。  
23. **Given** 模板已解除对某项目名称的引用，**When** 进入价目列表或触发绑卡同步，**Then** 该条目 `boundToCard` 自动为 false，无需手动刷新。
