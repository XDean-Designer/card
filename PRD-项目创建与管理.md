...
3. 项目可被会员卡权益 **按名称绑定**（项目桶、产品桶内 **名称唯一**）；绑卡后 **锁定名称、手机端可预约、删除**（优惠券/商城关联亦会锁定删除）；**在售/下架仍可改**。列表绑卡行以名称样式区分；删除锁定在左滑按钮与行操作 Sheet 以锁标呈现。`boundToCard` 随模板引用 **自动派生并刷新**。  
4. B 端改价、上下架、时长、隐藏、封面、**产品库存**等变更需 **同步客户端展示**（开单侧为 **实时目录/价格/库存**；推送细节【待联调确认】）；顾客端具体 UI **不在本文展开**。  
5. 会员卡建卡时「选项目 / 选产品 / 选折扣适用项目」及标题栏「管理价目表」依赖本价目数据；从权益页进入价目再返回时 **刷新权益选品分组 Tab**（重置为「全部」并重绘）；会员卡三步流程见 `PRD-会员卡管理.md`，本文只写联结面。  
6. **开单选品** 与权益选品同口径：仅 `onSale && !hidden`；**产品库存**参与加购上限（空库存=不限购，见 **8.1**；`stock` **预埋关联**后续「库存管理」）。开单过程中若价目改价导致应付变化，支付链路拦截提示（见 6.8）。  
...
| 可见性清晰 | 顾客端 / 开单 / 建卡选品范围分明（见模块 4.6；开单与权益同为在售且未隐藏） |
| 与卡对齐 | 绑卡锁定规则清晰；权益选品只取在售且未隐藏 |

## 1.3 本期做（In）

- 价目表列表：项目 Tab / 产品 Tab、标题栏品牌红填充「+添加」按钮（跟 Tab 直接新建、不弹窗；**无右下悬浮球**）、列头排序、空态（**无搜索**）  
- 列表分区：**在售区** → **「已下架」分区** →（「全部」Tab）底部 **「已隐藏」折叠**  
- 新增 / 编辑 / 删除项目与产品（表单三段：基本信息 / 售卖设置 / 高级设置）  
- **产品库存**（选填；列表列展示；开单加购/支付扣减见 **8.1**；**预埋关联后续「库存管理」**）  
- 上传封面图（选填，演示最多 1 张）  
- 在售切换、**隐藏 / 取消隐藏**  
...
- 项目绑卡锁定（部分字段）及提示文案  
- 从会员卡权益页「管理价目表」进入并返回  
- **开单选品** 联结（仅在售且未隐藏；产品带库存上限；价目改价支付拦截见 6.8）  
- B→C 同步生效的产品语义说明（无 C 端页面规格）

## 1.4 本期不做（Out）

- 顾客端 / 小程序预约页、开单页的完整 UI 规格（仅约定消费价目结果与库存规则）  
- **「库存管理」完整功能**（入库/出库/盘点/流水等）：**后续独立上线**；本期仅在价目产品预埋 `stock` 字段并约定关联语义（见 **8.1**）。工作台若已有「库存管理」入口，原型可不落地页面。  
...
| 对 C 端隐藏但 B 端保留 | **隐藏**（`hidden=true` / 系统「隐藏」组；「全部」底部「已隐藏」折叠；开单不可选） |
...
| 隐藏 | 对顾客端隐藏但 B 端保留 | `hidden=true`；「全部」主列表不显示、底部「已隐藏」折叠可展开；「隐藏」Tab 可见；开单不可选；**项目隐藏时关闭「手机端可预约」，取消隐藏后仍关，须手动再开** |
| 产品库存 | 选填库存以限制开单加购；预埋关联后续库存管理 | 见 **8.1**：空=不限购；支付扣减（办卡/续卡不扣）；与「库存管理」共用 `stock` 数量语义 |
...
价目表
 ├─ 列表（项目 Tab / 产品 Tab）
 │    ├─ 标题栏右侧品牌红「+添加」按钮（跟当前 Tab，不弹窗；无悬浮球）
...
| 页面/层 | 导航标题（原型） | 说明 |
|---------|------------------|------|
| 列表 | 价目表 | Tab 项目/产品；标题栏品牌红「+添加」按钮（跟当前 Tab，不弹窗；无悬浮球） |
...
- 项目隐藏时：**手机端可预约** 强制关闭并锁定；**取消隐藏后仍保持关闭**，须商户在详情中 **手动再打开**（不自动恢复）。
...
- **与预约**：凡使项目 `hidden=true`（左滑/行操作隐藏、加入系统「隐藏」组、编辑成员勾选进隐藏组等）⇒ `bookable=false`；取消隐藏 **不** 自动打开预约。
...
| 模式 | 入口 | 保存结果 |
|------|------|----------|
| 新增 | 标题栏「+添加」/ 空态 CTA（均跟当前项目或产品 Tab，**不弹选择层**；无悬浮球） | 写入对应 catalog；toast「项目已创建」/「产品已创建」；回列表 |
...
|------|--------|----------|-------------------------------|
| 在售且未隐藏 | 可见（项目预约另需 `bookable`） | 可选（产品另受库存上限） | 可选 |
| 已下架（含绑卡下架） | 不可见 | **不可选** | 不可选 |
| 隐藏（`hidden=true` / 系统「隐藏」组） | 不可见 | **不可选** | 不可选 |

**顾客端可见**（项目/产品）：`onSale && !hidden`。  
**开单选品**（项目/产品）：与权益同口径 `onSale && !hidden`（产品加购另见 **8.1** 库存规则）。  
...
| 产品库存非空但非 0～999999 整数 | 请填写有效库存（0～999999 的整数） |
| 产品库存超过 999999 | 库存不能超过 999999 |

时长以 **行内时长输入**（分/时切换）为主路径；上限按最多 **480 分钟（8 小时）** 约束。  
产品库存为 **选填**：空串 / 未填 → `stock=null`（不限购）；`0` 为有效值。
...
- 标题：价目表；右侧标题栏 **品牌红填充、白字「+添加」按钮**（约高 28、圆角 6；非纯文案链）  
  - **不弹窗 / 不选类型 Sheet**  
  - 当前 Tab 为 **项目** → 直接进入「新增项目」  
  - 当前 Tab 为 **产品** → 直接进入「新增产品」  
  - **无右下悬浮球「+」**（新建入口仅标题栏按钮 + 空态 CTA）  
...
- 表头可点循环排序：升序 → 降序 → 取消  
  ...
  - 产品列：`产品名称（规格）` / `库存` / `价格（¥）`（规格并入名称展示；库存列约 48px 居中）  
- 行：拖拽把手、名称（产品为「名称（规格）」；绑卡名称后跟 **14px 卡面图标**，非文字「绑卡」徽标）、时长（仅项目）、**库存（仅产品：有值显示数字含 0，空显示「—」）**、价格；**无行内状态胶囊**  
...
| 分区 | 字段 | 必填 | 默认（新增） | 说明 |
|------|------|:----:|--------------|------|
| 基本信息 | 项目名称 | ✓ | 空 | 最多 30 字；**同桶唯一**（trim 后）；绑卡后锁定；placeholder「请输入」 |
| 基本信息 | 项目价格 | ✓ | 空 | 元；金额键盘；≥0 |
| 基本信息 | 项目时长 | ✓ | 60 分钟 | 行内分/时输入；≥1 分钟 |
| 售卖设置 | 在售 | — | 开 | 绑卡 **不锁定** |
| 售卖设置 | 手机端可预约 | — | 开 | hint：打开后项目会在小程序-预约中显示；**下架、隐藏或绑卡时锁定**；隐藏解除后仍为关，须手动开 |
...
| 分区 | 字段 | 必填 | 默认（新增） | 说明 |
|------|------|:----:|--------------|------|
| 基本信息 | 产品名称 | ✓ | 空 | 表单标签「产品名称」；**同桶唯一**（trim 后）；校验空名时 toast「请填写产品名称」；重名 toast「名称已存在」 |
| 基本信息 | 规格 | — | 空 | placeholder「选填」；最多 20；空则列表仅显示名称 |
| 基本信息 | 产品价格 | ✓ | 空 | 同价格规则 |
| 基本信息 | 库存 | — | 空 | placeholder「选填」；0～999999 整数；空=`null`（开单不限购）；`0` 表示不可加购；与后续「库存管理」共用数量语义（见 8.1） |
| 售卖设置 | 在售 | — | 开 | |
...
| **开单选品** | 通过 `CardCatalogBridge.getBillProjects` / `getBillProducts` 读取价目实时目录，再过滤为 **仅** `onSale && !hidden`；产品行可展示「库存 N」；加购受库存上限（见 **8.1**）；改价后开单侧可见最新价；缓存/推送实现【待联调确认】 |
...
| price | number | |
| stock | number \| null | **选填**；`null`/未设 = 不限购；`0`～`999999` 为上限；列表空显示「—」；**与后续「库存管理」共用同一库存数量语义**（见 8.1） |
| onSale | boolean | |
| hidden | boolean | 同项目 |
...
| 开单价目桥接 | 实时目录供开单消费；开单侧再过滤 `onSale && !hidden`；产品含 `stock` |
| 库存数量读写 | 本期价目表单可写 `stock`；开单支付可扣减；后续由「库存管理」模块共管【待联调确认】 |
...
2. **开单选品范围**：与权益同口径，仅 `onSale=true && hidden=false`（下架/隐藏不可选）。  
3. **顾客端可见**：`onSale && !hidden`；项目预约另需 `bookable`。  
4. **产品库存**：仅产品具备；选填与开单限购、支付扣减规则见 **8.1**。  
...
12. **隐藏与预约**：项目隐藏 ⇒ `bookable=false` 且隐藏期间不可开；**取消隐藏后仍为关**，须手动再开「手机端可预约」。  
13. **B→C / 开单同步**：价目变更后客户端与开单侧按 **实时目录/价格/库存** 语义生效；推送与缓存细节【待联调确认】；顾客端页面结构不在范围。  
14. **列排序 vs 手动排序**：列排序为临时视图；手动拖拽前须取消列排序并回到「全部」分组。  
15. **产品无预约字段**；项目才有「手机端可预约」。  
...
适用范围：**仅产品**（项目无库存字段）。  
本期提供价目侧库存字段与开单限购/扣减；**「库存管理」完整能力后续上线**，与本字段关联（见下节）。

### 与后续「库存管理」的关联

| 项 | 约定 |
|----|------|
| 关联对象 | 价目 **产品** 的 `stock` 与后续「库存管理」中的 **可售/账面库存数量** 为 **同一业务量**（同一产品实体）。 |
| 关联键 | 以产品 `id`（及门店范围）对齐库存台账；名称/规格仅展示。【待联调确认】 |
| 本期（库存管理未上线） | 商户可在价目「新增/编辑产品」中 **手工维护** `stock`；空=`null` 表示未纳入库存管理（开单不限购）；开单支付成功可扣减该字段。 |
| 后续（库存管理上线后） | 入库、出库、盘点、调整等以 **库存管理** 为主入口更新数量；价目列表/表单的「库存」展示与读写须与库存管理 **保持一致**（价目侧是否仍允许直接改数、冲突策略、流水归属等由库存管理 PRD 定义，【待联调确认】）。 |
| 开单消费 | 无论库存由价目手工写入还是由库存管理回写，开单加购上限与支付扣减均消费 **当前 `stock`**（规则见本节其余小节）。 |
| 文档边界 | 库存管理的页面、权限、流水、预警等 **不在本文展开**；本文只锁定「价目产品字段 ↔ 库存数量」的预埋关联，避免后续上线时字段分裂。 |

### 字段与校验

| 项 | 规则 |
|----|------|
| 是否必填 | **选填**；表单 placeholder「选填」 |
| 取值 | 空串 / 未填 → `stock=null`；否则为 `0`～`999999` 整数 |
| 语义 | `null` = **不限购**（未纳入库存管理 / 未设置数量）；`0` = 可售上限为 0（不可加购）；`N>0` = 可购上限为 N |
| 非法输入 | Toast「请填写有效库存（0～999999 的整数）」或「库存不能超过 999999」 |

### 价目表展示

- 产品列表增加「库存」列（约 48px 居中）。  
- `stock=null` → 显示 **「—」**；有值（含 `0`）→ 显示数字。  
- 项目 Tab 不展示库存列。

### 开单加购

- 选品范围仍为 `onSale && !hidden`（与权益同口径）；库存只约束 **数量上限**，不单独决定是否出现在目录。  
- `stock=null`：不加购上限。  
- `stock=0`：不可加购；「+」禁用；Toast「库存不足」。  
- `stock=N`（N>0）：购物车数量 ≤ N；达上限「+」禁用；选品行可展示「库存 N」。  
- 试图超过上限：Toast「库存不足，最多 N 件」，数量钳制为 N。

### 支付前钳制与支付扣减

- 打开支付层 / 确认支付前，对购物车执行库存钳制（原型 `clampCartToProductStock`）：  
  - 上限为 `0` 的产品行从购物车移除；  
  - 数量大于上限的行钳制到上限；  
  - 钳制后购物车为空（且非资产/办卡等特殊路径）→ Toast「购物车为空」，中止支付；  
  - 仍存在超库存 → Toast「库存不足，请返回调整数量」，中止支付。  
- **支付成功后扣减**：按购物车数量减少对应产品 `stock`（不低于 0）；后续库存管理上线后，扣减须同步反映到库存台账【待联调确认】。  
- **不扣减**：办卡、续卡（及同路径不走普通账单扣库存的支付）。充值等非「产品购物车扣库存」路径亦不扣。  

交互与文案以原型为准；接口落库与并发扣减细节【待联调确认】。
...
**导航/按钮**：价目表、项目、产品、分组管理、编辑成员、新增项目、新增产品、项目详情、产品详情、保存、删除、新建分组、确定、取消、全选、反选、设置分组、下架、上架、隐藏、取消隐藏、分组、重命名、编辑成员、去新建分组、+添加、添加项目、添加产品、基本信息、售卖设置、高级设置、上传封面图（选填）、添加图片、在售、已下架、已隐藏、手机端可预约、规格、库存、时长（分钟）、价格（¥）、全部、知道了。

**Toast**：请输入分组名称、分组名称已存在、名称已存在、已重命名、已新建分组、已删除分组、已更新成员、已更新分组、已隐藏、已取消隐藏、已上架、已下架、保存成功、项目已创建、产品已创建、已删除、请填写项目名称、请填写产品名称、请填写有效价格、请填写项目时长、请填写有效库存（0～999999 的整数）、库存不能超过 999999、库存不足、库存不足，最多 N 件、库存不足，请返回调整数量、请清空搜索后再排序（分组筛选拦截拖拽时）、请先取消列排序后再手动排序、已添加演示图片、已移除图片、仅可上传 1 张封面、项目已隐藏，取消隐藏后需手动开启手机端可预约；以及模块 5.5 绑卡/预约/删除拦截句。
...
3. **Given** 产品 Tab，**When** 保存库存为空的产品，**Then** 列表库存列显示「—」；开单加购不限购。  
4. **Given** 产品库存为 0，**When** 开单对该产品点「+」，**Then** Toast「库存不足」且数量不加。  
5. **Given** 产品库存为 N（N>0），**When** 加购超过 N，**Then** Toast「库存不足，最多 N 件」且数量钳制为 N。  
...
11. **Given** 在售且可预约的项目，**When** 左滑隐藏或加入系统「隐藏」组，**Then** 「全部」主列表不显示但底部「已隐藏」折叠可展开；「隐藏」Tab 显示；顾客端与开单均不可见；**`bookable=false`**。  
12. **Given** 隐藏前可预约的项目，**When** 取消隐藏，**Then** 回到「全部」主列表可见（在售时顾客端可见）；**「手机端可预约」仍为关**，须手动再开。  
13. **Given** 新建分组并编辑成员勾选多项，**When** 确定，**Then** 按组筛选只显示非 hidden 成员。  
14. **Given** 条目属于两组，**When** 删除其中一组，**Then** 条目仍在，另一组关系保留。  
15. **Given** 在售区两行，**When** 拖拽换序，**Then** 刷新后顺序保持。  
16. **Given** 当前「隐藏」Tab 或自定义分组筛选，**When** 拖拽，**Then** toast「请清空搜索后再排序」。  
...
19. **Given** 已下架或已隐藏产品，**When** 开单选品，**Then** 列表中不出现该产品。  
20. **Given** 开单已选品并进入支付前，**When** 价目表改价导致应付与快照不一致，**Then** Dialog「支付金额已变化」；点「知道了」关闭支付层并留在开单详情。  
21. **Given** 开单含有库存产品且支付成功（非办卡/续卡），**When** 支付完成，**Then** 对应产品库存按数量扣减。  
22. **Given** 办卡或续卡支付成功，**When** 支付完成，**Then** **不**扣减产品库存。  
23. **Given** 项目详情高级设置，**When** 添加第 2 张封面，**Then** toast「仅可上传 1 张封面」。  
24. **Given** 「全部」列表有下架与隐藏条目，**When** 浏览，**Then** 无状态列；可见居中「已下架」分隔行（下沿线）与「已隐藏」折叠（无数量）。  
25. **Given** 当前为项目 Tab 且列表有数据，**When** 点标题栏「+添加」，**Then** **不弹窗**，直接进入「新增项目」。  
26. **Given** 当前为产品 Tab 且列表有数据，**When** 点标题栏「+添加」，**Then** **不弹窗**，直接进入「新增产品」。  
...
30. **Given** 产品表单，**When** 查看规格与库存输入框，**Then** placeholder 均为「选填」。
