...
- 价目表列表：项目 Tab / 产品 Tab、标题栏「+添加」（跟 Tab 直接新建、不弹窗）、有数据时右下悬浮球「+」、列头排序、空态（**无搜索**）  
...
价目表
 ├─ 列表（项目 Tab / 产品 Tab）
 │    ├─ 标题栏右侧「+添加」（跟当前 Tab，不弹窗）· 有数据时右下悬浮球「+」
...
| 页面/层 | 导航标题（原型） | 说明 |
|---------|------------------|------|
| 列表 | 价目表 | Tab 项目/产品；标题栏「+添加」（跟当前 Tab，不弹窗）；有数据时右下悬浮球「+」 |
...
| 模式 | 入口 | 保存结果 |
|------|------|----------|
| 新增 | 标题栏「+添加」/ 悬浮球「+」/ 空态 CTA（均跟当前项目或产品 Tab，**不弹选择层**） | 写入对应 catalog；toast「项目已创建」/「产品已创建」；回列表 |
...
- **无关键字搜索**（已移除搜索框）  
- 有数据时：右下 **悬浮球「+」**（品牌红圆 56、白加号；与标题栏相同，跟当前 Tab 直接新建；`aria-label` 为「添加项目」/「添加产品」；整表无数据时隐藏，改用空态 CTA）  
...
19. **Given** 当前为项目 Tab 且列表有数据，**When** 点标题栏「+添加」（或悬浮球），**Then** **不弹窗**，直接进入「新增项目」。  
20. **Given** 当前为产品 Tab 且列表有数据，**When** 点标题栏「+添加」（或悬浮球），**Then** **不弹窗**，直接进入「新增产品」。  
21. **Given** 整表无数据，**When** 查看列表，**Then** 无悬浮球，空态 CTA 可添加。
