...
6. **开单选品** 与权益选品同口径：仅 `onSale && !hidden`；**产品库存**参与加购上限（空库存=不限购，见 **8.1**）。开单过程中若价目改价导致应付变化，支付链路拦截提示（见 6.8）。  
...
- **产品库存**（选填；列表列展示；开单加购/支付扣减规则见 **8.1**）  
- 上传封面图（选填，演示最多 1 张）  
- 在售切换、**隐藏 / 取消隐藏**  
...
| 产品库存 | 选填库存以限制开单加购 | 见 **8.1**：空=不限购；`0` 不可加购；数量不超过库存；支付成功扣减（办卡/续卡不扣） |
...
| 基本信息 | 产品价格 | ✓ | 空 | 同价格规则 |
| 基本信息 | 库存 | — | 空 | placeholder「选填」；0～999999 整数；空=`null`（开单不限购）；`0` 表示不可加购 |
| 售卖设置 | 在售 | — | 开 | |
...
| price | number | |
| stock | number \| null | **选填**；`null`/未设 = 不限购；`0`～`999999` 为上限；列表空显示「—」 |
| onSale | boolean | |
| hidden | boolean | 同项目 |
...
适用范围：**仅产品**（项目无库存字段）。独立「库存管理」工作台模块 **本期不做**。
...
| 语义 | `null` = **不限购**（未管理库存）；`0` = 可售上限为 0（不可加购）；`N>0` = 可购上限为 N |
...
- **支付成功后扣减**：按购物车数量减少对应产品 `stock`（不低于 0）。  
