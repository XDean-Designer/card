...
6. **开单选品** 与权益选品同口径：仅 `onSale && !hidden`；价目产品 **不限购**（见 **8.1**）。开单过程中若价目改价导致应付变化，支付链路拦截提示（见 6.8）。  
...
- 价目表列表：项目 Tab / 产品 Tab、标题栏品牌红填充「+添加」按钮（跟 Tab 直接新建、不弹窗；**无右下悬浮球**）、列表底部虚线添加行、列头排序、空态（**无搜索**）  
- 列表分区：**在售区** → **「已下架」分区** →（「全部」Tab）底部 **「已隐藏」折叠**  
- 新增 / 编辑 / 删除项目与产品（表单三段：基本信息 / 售卖设置 / 高级设置；产品含 **规格选填**；**无库存字段**）  
...
- 项目绑卡锁定（部分字段）及提示文案  
- 从会员卡权益页「管理价目表」进入并返回  
- **开单选品** 联结（仅在售且未隐藏；价目产品不限购；价目改价支付拦截见 6.8）  
...
| 新建产品 | 填名称/规格（选填）/价格后出现在产品列表 | 列表「名称（规格）」+ 价格（无库存列）；在售未隐藏时可开单/权益选用 |
| 改价 | 绑卡或未绑卡均可改价（规则内） | toast 成功；客户端同步语义成立；开单中改价触发支付拦截（见 6.8） |
| 下架 | 停售但仍可在价目表看到 | 落入「已下架」分区；顾客端 / 开单 / 权益选品均不可见 |
| 隐藏 | 对顾客端隐藏但 B 端保留 | `hidden=true`；「全部」主列表不显示、底部「已隐藏」折叠可展开；「隐藏」Tab 可见；开单不可选；**项目隐藏时关闭「可预约」，取消隐藏后仍关，须手动再开** |
...
| 开单选品 | 开单只选可售货 | 仅 `onSale && !hidden`；价目产品不限购（8.1） |
...
价目表
 ...
 │    ├─ 在售区 →「已下架」分隔行 →（全部）「已隐藏」折叠 → 底部虚线添加行
 │    ├─ 行：点击进详情 · 拖拽排序 · 左滑/长按操作（产品：名称规格+价格）
 │    └─ 空态（保留空态 CTA）
 ├─ 新增项目 / 新增产品（基本信息含规格选填 · 售卖设置 · 高级设置·封面；无库存）
 ├─ 项目详情 / 产品详情
 ├─ 分组管理
 │    └─ 编辑成员（含系统「隐藏」组）
 └─ Sheets / 弹层
...
产品：绑卡名称样式同项目（列表名称 `is-bound` 样式区分，**无独立卡面图标**）；**在售/下架不受绑卡限制**；删除若同时关联优惠券/商城则 toast「已关联会员卡/优惠券/商城，不可删除」（按实际关联动态拼接）。产品详情不展示项目类通知条。
...
**开单选品**（项目/产品）：与权益同口径 `onSale && !hidden`（价目产品 **不限购**，见 **8.1**）。  
...
| 时长选择器确认时不足 5 分钟 | 时长至少5分钟 |

时长以 **行内时长输入**（分/时切换）为主路径；上限按最多 **480 分钟（8 小时）** 约束；选择器步进确认时至少 **5 分钟**。  
...
- 标题：价目表；右侧标题栏 **品牌红填充、白字「+添加」按钮**（约高 28、圆角 6；非纯文案链）  
  ...
  - **无右下悬浮球「+」**（新建入口：标题栏按钮 + 空态 CTA + **列表底部虚线「添加项目/添加产品」行**）  
- Tab：项目 | 产品（独立记住各 Tab 的当前分组筛选）  
- **无关键字搜索**（已移除搜索框）  
- 空态 CTA 仍为「添加项目」/「添加产品」（跟当前 Tab）  
- 可关闭提示条：`长按项目可拖动排序，左滑快捷操作；分组可左右滑动`（文案固定含「项目」；关闭状态以 session 记忆）  
- 分组轨：灰底 `#F2F2F2` 圆角轨道；`全部` + 自定义组 + 系统 **隐藏**（隐藏 Tab 带 hide 图标）；**未选** 灰字纯文字；**选中** 白胶囊 + **品牌红**字；可左右横滑，右侧渐隐；右侧 **深灰 18px** 齿轮进「分组管理」  
- **「全部」Tab 列表结构**（自上而下）：  
  1. 在售且未隐藏条目  
  2. **「已下架」分隔行**（仅当存在未隐藏的下架条目时出现）  
  3. 底部折叠条「**已隐藏**」（无数量）；展开后可查看/操作隐藏条目  
  4. （有数据时）底部虚线 **添加项目 / 添加产品** 行  
- 表头可点循环排序：升序 → 降序 → 取消  
  - 项目列：`项目名称` / `时长（分钟）` / `价格（¥）`（**无状态列**）  
  - 产品列：`产品名称（规格）` / `价格（¥）`（规格并入名称；**无库存列**）  
- 行：拖拽把手、名称（产品为「名称（规格）」；绑卡时名称 `is-bound` 样式，**无卡面图标**）、时长（仅项目）、价格；**无行内状态胶囊**  
...
| 售卖设置 | 在售 | — | 开 | 绑卡 **不锁定** |
| 售卖设置 | 可预约 | — | 开 | 表单标签为「可预约」；hint：打开后项目会在小程序-预约中显示；**下架、隐藏或绑卡时锁定**；隐藏解除后仍为关，须手动开 |
| 高级设置 | 项目封面图（可选） | — | 未上传 | 折叠轨 +「添加图片」；最多 1 张演示图；「未上传，不影响保存」 |
...
| 基本信息 | 产品价格 | ✓ | 空 | 同价格规则 |
| 售卖设置 | 在售 | — | 开 | |
| 高级设置 | 产品封面图（可选） | — | 未上传 | 同项目演示图规则 |

无时长、无预约开关、**无库存字段**（见 8.1）。分类默认「产品」。详情页不展示项目类通知条。
...
| **开单选品** | 通过 `CardCatalogBridge.getBillProjects` / `getBillProducts` 读取价目实时目录，再过滤为 **仅** `onSale && !hidden`；价目产品 **不限购**（`catalogStockLimit` 恒返回 null，见 **8.1**）；改价后开单侧可见最新价；缓存/推送实现【待联调确认】 |
...
| 开单价目桥接 | 实时目录供开单消费；开单侧再过滤 `onSale && !hidden`；价目产品不限购 |
...
开单选品范围仍为 `onSale && !hidden`。支付金额因价目改价不一致时，仍走 Dialog「支付金额已变化」（FLOW：`bill-pay-price-changed`，归属开单分组）。
...
**导航/按钮**：价目表、项目、产品、分组管理、编辑成员、新增项目、新增产品、项目详情、产品详情、保存、删除、新建分组、确定、取消、全选、反选、设置分组、下架、上架、隐藏、取消隐藏、分组、重命名、编辑成员、去新建分组、+添加、添加项目、添加产品、基本信息、售卖设置、高级设置、项目封面图（可选）/产品封面图（可选）、添加图片、在售、已下架、已隐藏、可预约、规格、时长（分钟）、价格（¥）、全部、添加项目、添加产品、知道了。

**Toast**：请输入分组名称、分组名称已存在、名称已存在、已重命名、已新建分组、已删除分组、已更新成员、已更新分组、已隐藏、已取消隐藏、已上架、已下架、保存成功、项目已创建、产品已创建、已删除、请填写项目名称、请填写产品名称、请填写有效价格、请填写项目时长、时长至少5分钟、「隐藏」分组内不支持拖拽排序，请切换到「全部」或其它分组、已隐藏的项目不支持拖拽排序、请先取消列排序后再手动排序、已添加演示图片、已移除图片、仅可上传 1 张封面、项目已隐藏，取消隐藏后需手动开启可预约、项目已下架，请先开启「在售」后再设置可预约、系统分组不可重命名或删除；以及模块 5.5 绑卡/预约/删除拦截句（删除可为「已关联会员卡/优惠券/商城，不可删除」动态拼接）。
...
22. **Given** 时长选择器确认，**When** 总时长不足 5 分钟，**Then** toast「时长至少5分钟」。  
