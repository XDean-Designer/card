...
- **操作紧凑**：搜索、Tab、分组条、左滑/长按菜单、拖拽排序集中在一屏；下架与隐藏以分区/折叠呈现；  
...
| 列表浏览与查找 | 搜索 + 列排序 + 分组筛选；信息行无配图、无状态列 |
...
| 模式 | 入口 | 保存结果 |
|------|------|----------|
| 新增 | 工具栏「添加」/ 空态 CTA | 写入对应 catalog；toast「项目已创建」/「产品已创建」；回列表 |
...
见模块 6.1；搜索无结果引导换关键词或清空。「隐藏」Tab 空态引导左滑将条目移入隐藏。
...
- 搜索：placeholder「搜索项目名称或价格」/「搜索产品名称或价格」；匹配 **名称**（产品另匹配 **规格**），不按价格匹配；右侧可有「搜索」按钮（聚焦并重渲）  
- 工具栏添加文案：**「添加」**（空态 CTA 仍为「添加项目」/「添加产品」）  
- 可关闭提示条：`长按项目可拖动排序，左滑快捷操作`（关闭状态以 session 记忆，刷新可再出）  
- 分组条：`全部` + 自定义组 + 系统 **隐藏**（隐藏 Tab 带 hide 图标；选中为白胶囊）+ 齿轮「分组管理」  
- **「全部」Tab 列表结构**（自上而下）：  
  1. 在售且未隐藏条目  
  2. **「已下架」分区**（分区头文案「已下架」，无数量；仅当存在未隐藏的下架条目时出现）  
  3. 底部折叠条「**已隐藏**」（无数量）；展开后可查看/操作隐藏条目；搜索命中隐藏项时自动展开  
- 表头可点循环排序：升序 → 降序 → 取消  
  - 项目列：`项目名称` / `时长（分钟）` / `价格（¥）`（**无状态列**）  
  - 产品列：`产品名称（规格）` / `价格（¥）`（规格并入名称展示，无独立规格列）  
- 行：拖拽把手、名称（产品为「名称（规格）」；绑卡名称加样式）、时长（仅项目）、价格；**无行内状态胶囊、无绑卡图标角标**  
...
| 条件 | 标题 | 说明 | CTA |
|------|------|------|-----|
| 整表无数据 | 暂无项目 / 暂无产品 | 项目：添加后可在选择项目与办卡中使用；产品：添加后可在开单记账中使用 | 添加项目 / 添加产品 |
...
- 「隐藏」分组 Tab 展示隐藏图标（`assets/catalog/hide.svg`）；列表底部「已隐藏」折叠 **不带** 该图标数量角标  
...
**导航/按钮**：价目表、项目、产品、分组管理、编辑成员、新增项目、新增产品、项目详情、产品详情、保存、删除、新建分组、确定、取消、全选、反选、设置分组、下架、上架、隐藏、取消隐藏、分组、重命名、编辑成员、去新建分组、添加、添加项目、添加产品、基本信息、售卖设置、高级设置、上传封面图（选填）、添加图片、在售、已下架、已隐藏、手机端可预约、规格、时长（分钟）、价格（¥）、全部、搜索。

**Toast**：请输入分组名称、分组名称已存在、已重命名、已新建分组、已删除分组、已更新成员、已更新分组、已隐藏、已取消隐藏、已上架、已下架、保存成功、项目已创建、产品已创建、已删除、请填写项目名称、请填写产品名称、请填写有效价格、请填写项目时长、请清空搜索后再排序、请先取消列排序后再手动排序、已添加演示图片、已移除图片、仅可上传 1 张封面；以及模块 5.5 绑卡/预约/删除拦截句。

**Dialog/Sheet**：确认删除？、删除后不可恢复，是否继续？、删除分组？、仅删除分组，不会删除项目/产品。、最多 20 字，不可与已有分组重名、可同时加入多个分组、暂无自定义分组、请先创建分组，再把项目/产品加入、长按项目可拖动排序，左滑快捷操作、打开后项目会在小程序-预约中显示、未上传，不影响保存；模块 6.2 三条详情通知。

**空态**：暂无项目/产品、暂无隐藏项目/产品、本组暂无项目/产品、暂无匹配项目/产品、暂无分组、创建分组后可快速筛选价目表、暂无可选项目/产品、可将价目表中的条目加入本组、左滑条目可将项目/产品移入隐藏、添加后可在选择项目与办卡中使用、添加后可在开单记账中使用、请在「管理价目表」中将项目/产品加入本组。
...
18. **Given** 「全部」列表有下架与隐藏条目，**When** 浏览，**Then** 无状态列；可见「已下架」分区头与「已隐藏」折叠（无数量）。
