...
| 拖拽排序 | 调整同售卖态桶内顺序 | 刷新后顺序保持；「隐藏」Tab / 「全部」已隐藏折叠 / 列排序激活时不可拖 |
...
不可跨桶；「隐藏」Tab / 「全部」已隐藏折叠内条目 / 列排序激活时拦截（文案见模块 5；**无搜索**）。
...
**Toast**：请输入分组名称、分组名称已存在、名称已存在、已重命名、已新建分组、已删除分组、已更新成员、已更新分组、已隐藏、已取消隐藏、已上架、已下架、保存成功、项目已创建、产品已创建、已删除、请填写项目名称、请填写产品名称、请填写有效价格、请填写项目时长、请填写有效库存（0～999999 的整数）、库存不能超过 999999、库存不足、库存不足，最多 N 件、库存不足，请返回调整数量、「隐藏」分组内不支持拖拽排序，请切换到「全部」或其它分组、已隐藏的项目不支持拖拽排序、请先取消列排序后再手动排序、已添加演示图片、已移除图片、仅可上传 1 张封面、项目已隐藏，取消隐藏后需手动开启手机端可预约；以及模块 5.5 绑卡/预约/删除拦截句。
