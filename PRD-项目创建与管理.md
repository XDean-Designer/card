...
| 8 | 业务规则 | ✓ | **必看** | ✓ | 排序桶、多分组、可见性、库存、改价支付拦截 |
...
6. **开单选品** 与权益选品同口径：仅 `onSale && !hidden`；**产品库存**参与加购上限（空库存=不限购，见模块 8）。开单过程中若价目改价导致应付变化，支付链路拦截提示（见 6.8）。  
...
- **产品库存**（选填；列表列展示；开单加购/支付扣减规则见模块 8）  
- 上传封面图（选填，演示最多 1 张）  
- 在售切换、**隐藏 / 取消隐藏**  
...
| 产品库存 | 选填库存以限制开单加购 | 空=不限购；`0` 不可加购；数量不超过库存；支付成功扣减（非办卡/充值） |
...
**开单选品**（项目/产品）：与权益同口径 `onSale && !hidden`（产品加购另见模块 8 库存规则）。  
...
| **开单选品** | 通过 `CardCatalogBridge.getBillProjects` / `getBillProducts` 读取价目实时目录，再过滤为 **仅** `onSale && !hidden`；产品行可展示「库存 N」；加购受库存上限（见模块 8）；改价后开单侧可见最新价；缓存/推送实现【待联调确认】 |
...
4. **产品库存（选填）**：  
   ...
   - `stock>0` → 购物车数量 ≤ 库存；超限 Toast「库存不足，最多 N 件」并钳制；列表/选品可展示库存；  
   - 打开支付 / 确认前 `clampCartToProductStock`；支付成功（非办卡/充值）`deductProductStockAfterPay` 扣减库存。  
...
21. **Given** 开单含有库存产品且支付成功（非办卡/充值），**When** 支付完成，**Then** 对应产品库存按数量扣减。  
22. **Given** 项目详情高级设置，**When** 添加第 2 张封面，**Then** toast「仅可上传 1 张封面」。  
23. **Given** 「全部」列表有下架与隐藏条目，**When** 浏览，**Then** 无状态列；可见居中「已下架」分隔行（下沿线）与「已隐藏」折叠（无数量）。  
24. **Given** 当前为项目 Tab 且列表有数据，**When** 点标题栏「+添加」，**Then** **不弹窗**，直接进入「新增项目」。  
25. **Given** 当前为产品 Tab 且列表有数据，**When** 点标题栏「+添加」，**Then** **不弹窗**，直接进入「新增产品」。  
26. **Given** 任意价目表列表态，**When** 查看界面，**Then** **无**右下悬浮球「+」；整表无数据时可用空态 CTA 添加，标题栏「+添加」始终可用。  
27. **Given** 同桶已存在名称「精致剪发」，**When** 再保存同名项目（trim 后相同），**Then** toast「名称已存在」且不写入。  
28. **Given** 模板已解除对某项目名称的引用，**When** 进入价目列表或触发绑卡同步，**Then** 该条目 `boundToCard` 自动为 false，无需手动刷新。  
29. **Given** 产品表单，**When** 查看规格与库存输入框，**Then** placeholder 均为「选填」。
