...
| 隐藏 | 对顾客端隐藏但 B 端保留 | `hidden=true`；「全部」主列表不显示、底部「已隐藏」折叠可展开；「隐藏」Tab 可见；开单不可选 |
...
12. **B→C / 开单同步**：价目变更后客户端与开单侧按 **实时目录/价格/库存** 语义生效；推送与缓存细节【待联调确认】；顾客端页面结构不在范围。  
13. **列排序 vs 手动排序**：列排序为临时视图；手动拖拽前须取消列排序并回到「全部」分组。  
14. **产品无预约字段**；项目才有「手机端可预约」。  
15. **权益选品分组 Tab**：仅自定义组 +「全部」；**不展示** 系统「隐藏」Tab；从「管理价目表」返回后 **刷新分组 Tab（重置为「全部」）**。  
16. **封面图**：选填；演示最多 **1 张**；正式 OSS **本期不做**。  
17. **权限**：店主可编辑价目；店员不可编辑。  
18. **门店范围**：**本期仅单店**。
...
**Toast**：请输入分组名称、分组名称已存在、名称已存在、已重命名、已新建分组、已删除分组、已更新成员、已更新分组、已隐藏、已取消隐藏、已上架、已下架、保存成功、项目已创建、产品已创建、已删除、请填写项目名称、请填写产品名称、请填写有效价格、请填写项目时长、请填写有效库存（0～999999 的整数）、库存不能超过 999999、库存不足、库存不足，最多 N 件、库存不足，请返回调整数量、请清空搜索后再排序（分组筛选拦截拖拽时）、请先取消列排序后再手动排序、已添加演示图片、已移除图片、仅可上传 1 张封面；以及模块 5.5 绑卡/预约/删除拦截句。
...
11. **Given** 条目，**When** 左滑隐藏或加入系统「隐藏」组，**Then** 「全部」主列表不显示但底部「已隐藏」折叠可展开；「隐藏」Tab 显示；顾客端与开单均不可见。  
12. **Given** 隐藏条目，**When** 取消隐藏，**Then** 回到「全部」主列表可见（在售时顾客端可见）。  
