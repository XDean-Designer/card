...
**ticket 视觉（与原型一致）**：柔和票券样式——左右半圆 notch、虚线撕口、右侧票根约 **90px**（次数/数量摘要与删除）、左侧短品牌色条与轻阴影。项目有赠送时票根为两行：`N次` / `送M次`（无「+」）。
...
- 面值金额（可编，可为 0）  
- 赠送（只读）= max(0, 面值 − 购买金额)  
- **面值可用于支付办卡/购卡费用**（含被购卡本身未配置面值权益的情况）；**不可用于购买价目「产品」**；Step2 无「可购买产品」配置项。
...
- 选品页计次模式为「每项 / 共计」；**保存后**：  
  ...
  - **共计** → 共享次数池合并为一张 ticket；票面 scope 文案为 **「共」**（非「共计」）  
- 有赠送时票根两行展示购买次数与「送M次」。
...
面值 **可用于支付办卡/购卡费用**（含被购卡无面值权益）；**不可**用于购买价目「产品」（不以面值购价目产品）。
...
| 4b | 面值可办卡 | 会员有面值余额 | 结账用会员卡支付办卡（含无面值卡） | 可用面值抵扣办卡款 |
...
- 价目：项目/产品勾选依赖价目数据；价目产品本身不在本文交付。面值可用于办卡/购卡，不可用于购买价目产品。  
- 会员：办卡单选既存会员。  
- 开单结账：办卡收款与支付成功回调。  
- 支付/退款通道：`【待联调确认】`。
