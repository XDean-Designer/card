...
- **仅已下架有底栏**：重新上架 | 已持卡管理  
- 运营数据 + `?` 说明（定义见模块 9）  
- 卡面信息：面值金额 / 赠送（不含可否购产品）；项目 / 产品 / 折扣摘要；有期限权益时展示 **「项目/折扣有效期」**（含产品开启场景）；用卡策略展示为 `均值退卡 · 均值计绩` 等形式，退卡/业绩各带 `?`  
...
办卡：IssuePicker(单选) → 确认办卡(员工) → 去结账 → PaySuccess → FinalizeIssue → IssueSuccess
...
原型「有数据」注入演示卡 + 价目项目（含产品 Tab）。正式环境自备等价数据；演示开关不进产品。
