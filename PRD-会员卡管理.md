...
5. **办卡**：选择会员 Sheet **单选**一位未持卡会员 → **「去结账」** → **确认办卡**（卡已锁定；选服务员工）→ 开单结账完成支付 → 办卡成功（来自结账时底栏可为「返回开单」）。  
...
说明：选择会员后「去结账」进入 **确认办卡** 页（不复用开单「添加卡」）；确认后开单结账。退卡走「退卡估值」Sheet。编辑锁定、Tab 在售/已下架等见模块 4。
...
**卡面摘要面板**（列表与详情「卡面信息」共用同一套规则；合并粒度对齐 Step2 权益 ticket）
...
- 期限：有项目/折扣/产品展示卡有效期；纯面值「永久有效」；否则「—」  
...
- **仅已下架有底栏**：重新上架 | 已持卡管理  
- 运营数据 + `?` 说明（定义见模块 9）  
- 卡面信息含面值金额/赠送（不含可否购产品）；**项目/产品/折扣行规则同 6.1**（含共计合并、折扣同规则合并、`名称 · 参数`）；超 4 展开；用卡策略各段可 `?`
