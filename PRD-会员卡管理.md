...
- 权益组合：面值（金额与赠送；**不可**用于购买价目产品）、项目（每项/共计、购买/赠送/不限次）、**产品（每项数量，不可不限量）**、折扣（比例/固定金额）；均可配置多张权益 ticket   
...
| 旧体系常见能力（语义） | 新体系如何配置 |
|------------------------|----------------|
| 储值/充值类（买赠面值） | 开启 **面值**：填面值金额；赠送 = 面值 − 购买金额（自动）；面值 **不可**用于购买价目产品 |
...
| 产品权益页 | 添加/编辑产品权益 | 适用产品 + 分组筛选 + **数量**；页内「确定」；「管理价目表」 |
...
| 添加产品权益 | 选产品 + 数量 | [demo.html?flow=pick-products](demo.html?flow=pick-products) |
...
三类 Switch 可多开；每类可多张 ticket（点票面编辑；项目/产品/折扣 ticket 右侧灰色描线垃圾桶删除，内容垂直居中）。
...
- 面值金额（可编，可为 0）  
- 赠送（只读）= max(0, 面值 − 购买金额)  
- **面值不可用于购买价目「产品」**；Step2 无「可购买产品」配置项。
...
- +添加产品权益；ticket 展示「每项」+ 数量摘要（如 `×3`）。  
- 参数：**仅一个「数量」**（不拆购/赠）；多选时每个产品各自获得该数量；**禁止不限量**。  
...
- 适用产品：仅在售；**反选 | 全选** 仅作用于当前分组 Tab  
- **数量设置**：单一数量步进器（≥1）；应用到每个已选产品（每项）；无「共计」、无「不限量」  
...
面值 **不可**用于购买价目「产品」（仅储值类消费语义，不以面值购产品）。
...
| 列表操作 | +复制建卡；办卡/退卡 |
| 展开 | 展开 N 项；收起 |
| 权益页 | 管理价目表；分组 Tab（全部 + 自定义组）；本组暂无项目/产品；反选；全选；数量；确定 |
...
- 价目：项目/产品勾选依赖价目数据；价目产品本身不在本文交付。面值不可用于购买产品。  
- 会员：办卡单选既存会员。  
- 开单结账：办卡收款与支付成功回调。  
- 支付/退款通道：`【待联调确认】`。
