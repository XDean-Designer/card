...
- 列表/详情卡面权益行与 Step2 ticket **同组合并**（共计多名一行；折扣同规则多名一行；文案多为 `名称 · 参数`；细则见完整稿 §6.1）  
...
| 次卡/项目次数类 | 开启 **项目**：选适用项目；每项或共计；购买/赠送次数或不定次；保存后每项 = 一项目一 ticket；**共计** → 共享池一票（票根「共」、卡面「共计」） |
| 产品次数/礼品类 | 开启 **产品**：行内设数量（qty≥1 为已选）；一种产品一张 ticket；禁止不限量 |
| 折扣/会员价类 | 开启 **折扣**：选适用项目；标尺折扣或固定金额；可一张 ticket 多项目；卡面按同规则合并展示 |
...
| 页面/层 | 导航标题（原型） | 说明 |
|---------|------------------|------|
| 列表 | 会员卡管理 | Tab：在售 / 已下架；底栏「添加会员卡」；空态可点插图创建；权益摘要按完整稿卡面规则合并展示 |
...
| 创建成功 | — | 返回列表 / 立即为会员办卡 |
| 详情 | 会员卡详情 | 运营数据 + 卡面信息（合并规则同列表）；在售顶栏下架/编辑 |
